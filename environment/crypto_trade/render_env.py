...
if __name__ == "__main__":
    # Inicializar el gráfico de trading con un símbolo
    trading_graph = TradingGraph(sym='BTC-USD')

    # Generar datos iniciales para la gráfica
    initial_data = np.random.randn(trading_graph.screen_size) * 100 + 10000
    trading_graph.reset_render_data(initial_data)

    # Simular la actualización de la gráfica con nuevos datos
    for _ in range(500):
        # Simular un nuevo precio del midpoint
        new_midpoint = 10000 + np.random.randn() * 100
        # Renderizar la gráfica con el nuevo precio
        trading_graph.render(midpoint=new_midpoint)
