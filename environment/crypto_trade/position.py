...
from environment.crypto_trade.order import MarketOrder, LimitOrder
from environment.crypto_trade.statistics import TradeStatistics
...
class Position(object):
    ...
    def cancel_limit_order(self) -> bool:
        ...
        """
        if self.order is None:
            print('No {} open orders to cancel.'.format(self.side))
            return False

        print('Cancelling order ({})'.format(self.order))
        self.order = None
        return True
    ...
    def remove(self, netting_order: MarketOrder or LimitOrder) -> float:
        ...
        if self.position_count < 1:
            print('Error. No {} positions to remove.'.format(self.side))
            return pnl
...
if __name__ == "__main__":
    # Crear una instancia de Position

    # Configuración inicial
    position = Position(side='long', buy_fee=0.01, sell_fee=0.015, max_position=1, transaction_fee=True)

    # Agregar órdenes
    market_order = MarketOrder(ccy='BTC', side='long', price=10000.0)
    position.add(market_order)



    # Procesar pasos
    position.step(bid_price=9700.0, ask_price=9750.0, buy_volume=0.5, sell_volume=0.5, step=1)



    # Eliminar posición
    netting_order = MarketOrder(ccy='BTC', side='short', price=9800.0)
    pnl = position.remove(netting_order)
    print(f"PnL from removing position: {pnl}")

    # Obtener PnL no realizado
    unrealized_pnl = position.get_unrealized_pnl(price=9900.0)
    print(f"Unrealized PnL: {unrealized_pnl}")

    # Aplanar inventario
    flatten_pnl = position.flatten_inventory(price=9900.0)
    print(f"PnL from flattening inventory: {flatten_pnl}")

    # Obtener distancia al punto medio
    distance_to_midpoint = position.get_distance_to_midpoint(midpoint=9800.0)
    print(f"Distance to midpoint: {distance_to_midpoint}")

    # Estado actual del objeto Position
    print(position)
