...
if __name__ == "__main__":
    ...
    visualizer = Visualize(columns=columns, store_historical_observations=True)

    # Agregar observaciones de ejemplo
    for _ in range(100):
        obs = np.random.rand(5)
        visualizer.add_observation(obs)

    # Agregar datos de ejemplo
    for i in range(100):
        midpoint = 10000 + np.random.randn() * 100
        buys = np.random.randint(0, 2)
        sells = np.random.randint(0, 2)
        inventory = np.random.randint(-10, 10)
        realized_pnl = np.random.randn() * 0.1
        visualizer.add(midpoint, buys, sells, inventory, realized_pnl)
        visualizer.plot_episode_history(save_filename='episode_history')
